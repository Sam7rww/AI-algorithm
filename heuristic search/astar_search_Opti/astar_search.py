...
class Node:
    ...
    def __init__(self, city, cost=0, h_val=0.0, parent=None, path=[]):
        self.city = city
        self.cost = cost
        self.h_val = h_val
        self.parent = parent
...
def A_star(cityS, cityE):
    ...
    """
    node = Node(city=cityS, cost=0, h_val=cal_h_value(cityS, cityE), path=[])
    frontier = [node]
    ...
    while frontier:
        ...
        for i in range(len(next_city_list)):
            ...
            if next_city not in node.path:
                ...
                child = Node(city=next_city, cost=0, h_val=h_value, parent=node, path=node.path.copy())
                child.path.append(node.city)
                child.cost = node.pathCost(child, distance)
                # print("Adding:", child.city, " h_val is:", child.h_val)
                if (next_city not in reached) or (child.A_star_val() < reached[next_city].A_star_val()):
                    reached.update({next_city: child})
